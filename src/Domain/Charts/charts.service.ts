...
import { Validator } from './charts.validator'
...
export class Service {
  ...
  public async findCharts(req: Req): Promise<Record<string, any>> {
    const { filters, ...options } = await this.$filters.handle(req)

    const _filters = filters

    const found = await this.$repo.findProductiveChain(options, _filters)
